...
namespace {
    if (!function_exists('env')) {
        ...
        {
            $env = \PHPExperts\RESTSpeaker\env($key, $default, '');

            return $env === '' ? $env : 'm-' . $env;
        }
    }
}
...
namespace PHPExperts\RESTSpeaker\Tests {
    ...
    class RESTAuthTest extends TestCase
    {
        ...
        public function testHasItsOwnEnvPolyfill()
        {
            // Because composer doesn't autoload the file for the function.
            require_once __DIR__ . '/../src/RESTAuth.php';
            ...
            $key = 'key';
            $expected = 'mock-key';
            $this->assertEquals($expected, \PHPExperts\RESTSpeaker\env($key, '', $env));

            // Will use the pollyfill env() by default, if env() is not available.
            putenv('key=mock-key');
            $this->assertEquals($expected, \PHPExperts\RESTSpeaker\env($key, '', ''));

            // Will use the native env() by default, if env() is available.
            putenv('key=mock-key2');
            $this->assertEquals('m-mock-key2', \PHPExperts\RESTSpeaker\env($key));
        }
    }
}
